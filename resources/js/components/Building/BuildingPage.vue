    ...
    >
      <template #additional-header>
        <button class="btn btn-success" @click="openCreateModal">
          <i class="fal fa-plus"></i> Create Building
        </button>
      </template>
...
const datatableHeaders = [
  ...
  { text: 'Campus', value: 'campus_name', width: '25%', sortable: false },
...
const datatableFetchUrl = '/api/buildings'
...
const openCreateModal = () => {
  ...
  buildingModal.value.show({ isEditing: false })
}

const openEditModal = async (building) => {
  try {
    const response = await axios.get(`/api/buildings/${building.id}/edit`)
    const fullBuilding = response.data.data || {}
    ...
    console.error('Failed to fetch building data for editing:', error)
    showAlert('Error', 'Failed to load building data for editing.', 'danger')
  }
}
...
const handleDelete = async (building) => {
  ...
  try {
    ...
    showAlert('Deleted', `"${building.name}" was deleted successfully.`, 'success')
    reloadDatatable()
  } catch (e) {
    console.error(e)
