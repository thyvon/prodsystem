        ...
        <div class="row justify-content-center">
          ...
          <div class="col-md-3 mb-4">
            <div class="card border shadow-sm h-100">
              <div class="card-body">
                <label class="font-weight-bold d-block mb-2">Requested By</label>
                <img :src="`/storage/${report.creator_profile_picture}`" class="rounded-circle mb-2" width="50" height="50">
                ...
                <div class="font-weight-bold">{{ report.prepared_by ?? 'N/A' }}</div>
                <p class="mb-1">
                  Status: 
                  <span class="badge badge-success">Requested</span>
                </p>
                <p class="mb-1 text-start">Position: {{ report.creator_position ?? 'N/A' }}</p>
                <p class="mb-0">Date: {{ formatDateTime(report.report_date) || 'N/A' }}</p>
              </div>
            </div>
          </div>
          ...
          <div v-for="(approval, i) in approvals || []" :key="i" class="col-md-3 mb-4">
            <div class="card border shadow-sm h-100">
              <div class="card-body">
                <label class="font-weight-bold d-block mb-2">{{ approval.request_type_label_en }}</label>
                ...
                <p class="mb-1 text-start">Position: {{ approval.position_name ?? 'N/A' }}</p>
...
const submitApproval = async (action) => {
  ...
  try {
    const res = await axios.post(
      `/api/inventory/stock-reports/report/${props.warehouseProductReportId}/submit-approval`,
      { request_type: report.value.approval_buttons.requestType, action: action, comment: commentInput.value?.trim() || '' }
    );
...
const confirmReassign = async () => {
  ...
  if (!report.value.approval_buttons?.requestType) { showAlert('Error', 'Request type not found.', 'danger'); return; }
  try {
    await axios.post(`/api/inventory/stock-reports/report/${props.warehouseProductReportId}/reassign-approval`, {
...
const openReassignModal = async () => {
  try {
    const res = await axios.get(`/api/inventory/stock-reports/report/get-approval-users`)
