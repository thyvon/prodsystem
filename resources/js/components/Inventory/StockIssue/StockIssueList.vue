...
const importFileAction = async () => {
  ...
  try {
    ...
    const response = await axios.post('/api/inventory/stock-issues/import', formData, {
      headers: { 'Content-Type': 'multipart/form-data' }
    })
    showAlert('Success', response.data.message || 'Import completed!', 'success')
    datatableRef.value?.reload()
    $(importModal.value).modal('hide')
    importFile.value = null
    importFileName.value = ''
    $(importFileInput.value).val('') // reset input
  } catch (error) {
    console.error(error)
    showAlert('Error', error.response?.data?.message || 'Something went wrong.', 'danger')
  } finally {
