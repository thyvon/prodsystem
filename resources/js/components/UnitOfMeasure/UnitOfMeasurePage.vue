    ...
    >
      <template #additional-header>
        <button class="btn btn-success" @click="openCreateModal">
          <i class="fal fa-plus"></i> Create Unit of Measure
        </button>
      </template>
...
import UnitOfMeasureModal from './UnitOfMeasureModal.vue'
import { confirmAction, showAlert } from '@/Utils/bootbox'

// Refs and state
const datatableRef = ref(null)
const unitOfMeasureModal = ref(null)
...
[
  ...
  { text: 'Parent Unit', value: 'parent_unit_name', width: '20%', sortable: false },
  { text: 'Operator', value: 'operator', width: '10%', sortable: true },
  { text: 'Conversion Factor', value: 'conversion_factor', width: '10%', sortable: true },
...
const datatableFetchUrl = '/api/unit-of-measures'
...
const openCreateModal = () => {
  ...
  unitOfMeasureModal.value.show({ isEditing: false })
}

const openEditModal = async (unitOfMeasure) => {
  try {
    const response = await axios.get(`/api/unit-of-measures/${unitOfMeasure.id}/edit`)
    const fullUnitOfMeasure = response.data.data || {}
    isEditing.value = true
    unitOfMeasureModal.value.show({
      isEditing: true,
      ...fullUnitOfMeasure
    })
  } catch (error) {
    console.error('Failed to fetch unit of measure data for editing:', error)
    showAlert('Error', 'Failed to load unit of measure data for editing.', 'danger')
  }
}
...
const handleDelete = async (unitOfMeasure) => {
  const confirmed = await confirmAction(
    `Delete "${unitOfMeasure.name}"?`,
    '<strong>Warning:</strong> This action cannot be undone!'
  )
  if (!confirmed) return

  try {
    await axios.delete(`/api/unit-of-measures/${unitOfMeasure.id}`)
    showAlert('Deleted', `"${unitOfMeasure.name}" was deleted successfully.`, 'success')
    reloadDatatable()
  } catch (e) {
    console.error(e)
