    ...
    >
      <template #additional-header>
        <button class="btn btn-success" @click="openCreateModal">
          <i class="fal fa-plus"></i> Create Attribute
        </button>
      </template>
...
const datatableHeaders = [
  ...
  { text: 'Ordinal', value: 'ordinal', width: '35%', sortable: true },
...
const datatableFetchUrl = '/api/product-variant-attributes'
...
const openCreateModal = () => {
  ...
  attributeModal.value.show({ isEditing: false })
}

const openEditModal = async (attribute) => {
  try {
    const response = await axios.get(`/api/product-variant-attributes/${attribute.id}/edit`)
    const fullAttribute = response.data.data || {}
    ...
    console.error('Failed to fetch attribute data for editing:', error)
    showAlert('Error', 'Failed to load attribute data for editing.', 'danger')
  }
}
...
const handleDelete = async (attribute) => {
  ...
  try {
    await axios.delete(`/api/product-variant-attributes/${attribute.id}`)
    showAlert('Deleted', `"${attribute.name}" was deleted successfully.`, 'success')
    reloadDatatable()
  } catch (error) {
    console.error(error)
