    ...
    >
      <template #additional-header>
        <button class="btn btn-success" @click="openCreateModal">
          <i class="fal fa-plus"></i> Create Campus
        </button>
      </template>
...
import CampusModal from './CampusModal.vue'
import { confirmAction, showAlert } from '@/Utils/bootbox'

// Refs and state
const datatableRef = ref(null)
const campusModal = ref(null)
...
const datatableHeaders = [
  ...
  { text: 'Address', value: 'address', width: '25%', sortable: false },
...
const datatableFetchUrl = '/api/campuses'
...
const openCreateModal = () => {
  ...
  campusModal.value.show({ isEditing: false })
}

const openEditModal = async (campus) => {
  try {
    const response = await axios.get(`/api/campuses/${campus.id}/edit`)
    const fullCampus = response.data.data || {}
    isEditing.value = true
    campusModal.value.show({
      isEditing: true,
      ...fullCampus
    })
  } catch (error) {
    console.error('Failed to fetch campus data for editing:', error)
    showAlert('Error', 'Failed to load campus data for editing.', 'danger')
  }
}
...
const handleDelete = async (campus) => {
  ...
  try {
    ...
    showAlert('Deleted', `"${campus.name}" was deleted successfully.`, 'success')
    reloadDatatable()
  } catch (e) {
    console.error(e)
