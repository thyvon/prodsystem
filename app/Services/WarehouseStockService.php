...
class WarehouseStockService
{
    ...
    public function calculateProductStock(WarehouseProduct $product, int $warehouseId, Carbon $threeMonthsAgo, Carbon $sixMonthsAgo): array
    {
        ...
        return array_merge($product->toArray(), [
            ...
            'avg_price' => number_format($avgPrice, 4, '.', ''),
            ...
            'ending_stock_cover_days' => round($endingStockCoverDays, 0),
            'buffer_15_days_qty' => round($buffer15DaysQty, 0),
            'order_lead_time_ss_days' => round($orderLeadTimeSafetyDays, 0),
            'safety_stock_qty' => round($safetyStockQty, 0),
            'stock_in_days' => round($stockInDays, 0),
            'target_safety_stock_days' => round($targetSafetyStockDays, 0),
            'ending_stock_qty' => round($endingStockQty, 0),
            'stock_value_usd' => round($stockValueUSD, 2),
            'inventory_reorder_qty' => round($inventoryReorderQty, 0),
            'reorder_level_qty' => round($reorderLevelQty, 0),
            'max_inventory_level_qty' => round($maxInventoryLevelQty, 0),
            'max_usage_days' => round($maxUsageDays, 0),
